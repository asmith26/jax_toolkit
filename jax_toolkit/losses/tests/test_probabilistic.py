...
class TestKullbackLeiblerDivergence(unittest.TestCase):
    ...
    def test_multiclass_returns_correctly(self):
        ...
        y_pred = jnp.asarray([.4, .9, .12, .36, .3, .4]).reshape((2, 3))
        y_true = jnp.asarray([.5, .8, .12, .7, .43, .8]).reshape((2, 3))
    ...
    def test_raises_when_number_of_samples_not_equal_multiclass(self):
        with self.assertRaises(TypeError) as _:
            kullback_leibler_divergence(jnp.array([[-1, 1], [1, -1]]), jnp.array([[-0.2, 0.7], [0.6, -0.5], [0.4, 0.1]]))
